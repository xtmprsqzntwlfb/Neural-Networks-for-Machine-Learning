...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  ret.input_to_hid = (model.hid_to_class'*(class_prob - data.targets).*(hid_output.*(1-hid_output))*data.inputs') / size(data.targets, 2) + wd_coefficient*model.input_to_hid;

  ret.hid_to_class = ((class_prob - data.targets)*hid_output') / size(data.targets, 2) + wd_coefficient*model.hid_to_class;
end
